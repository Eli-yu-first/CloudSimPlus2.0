 ...
 */
public class DynamicCloudletsArrival2test_10 {
...
//  Vm vm = bindCloudletToVm(this.vmList, cloudlet, broker, CloudletToVM_GREEDY_Hy); // 轮询算法
    ...
    submissionDelay += random.nextDouble(submissionTime);//默认是10 9-15

    return cloudlet;
  }
    /** Default constructor that builds and starts the simulation. */
    private DynamicCloudletsArrival2test_10() throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        for(int k =0;k<=3;k++){
            ...
            for(int j = 2;TF <= 15;j++){
                ...
                CLOUDLETS_NUMBER = 300;
                submissionTime = TF;
                for (int i = 0; i < testTimes; ++i) {
                    new DynamicCloudletsArrival2test_10();
                }
...
//    }
    public static void dataToExcel(int SheetIndex,int rowIndex,int comChar, double value) throws IOException {
        ...
        int comid = comChar ;
//        int comid = comChar-'A';
        File file1 = new File("D:\\testData\\" + "Task_frequency.xlsx");
        if (!file1.exists()) {
