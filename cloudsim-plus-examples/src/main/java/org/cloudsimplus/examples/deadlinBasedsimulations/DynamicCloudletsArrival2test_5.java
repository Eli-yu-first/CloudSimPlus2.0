 ...
 */
public class DynamicCloudletsArrival2test_5 {
  ...
  private static final int CLOUDLETS_NUMBER = 50;
  ...
  private static double[] Cost = new double[VMS_NUMBER];
  ...
  private DynamicCloudletsArrival2test_5() throws IOException {
  ...
  public static void main(String[] args) throws IOException {
    for (int i = 0; i < testTimes; ++i) {
      new DynamicCloudletsArrival2test_5();
    }
    ...
    System.out.printf("平均使用率为: Max_CPU_Usage: %5.2f ,Max_Ram_Usage:%5.2f ,Max_Bw_Usage:%5.2f %n", Max_CPU_usage, Max_Ram_usage, Max_Bw_usage);
  }
    ...
    private Vm bindCloudletToVm_GREEDY_Hy(Cloudlet cloudlet, List<Vm> vmList) {
        ...
        for (Vm vms : vmList) {
            if(getPretimeWithTrans(cloudlet,vms) < minTime){
                minTime = getPretimeWithTrans(cloudlet,vms);
                tmp = vms;
            }
        }
        return tmp;
...
//          host.setPowerModel(powerModel);
  ...
  private double getPretime(Cloudlet cloudlet, Vm vm) {
    ...
    private double getPretimeWithTrans(Cloudlet cloudlet, Vm vm) {
        ...
        for (Cloudlet CL : CloudletWaitingList) {
            if (CL.getVm() == this) {
                preWaitTime +=( CL.getLength()+CL.getFileSize()) / vm.getMips();
            }
        }
        double execTime = 1.0 * cloudlet.getLength() / vm.getMips();
        return preWaitTime + execTime;
